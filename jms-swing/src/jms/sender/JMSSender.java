...
public class JMSSender extends Thread{
	 ...
	 * @param queueName полное им€ очереди сообщени€ в которое посылаетс€ сообщение
	 * @param senderTitle - подпись отправител€ 
	 * Ќеобходим ќЅя«ј“≈Ћ№Ќџ… старт методом start()
	 * */
	public JMSSender(String pathToJMS, 
					 String queueName, 
					 int maxQueueSize,
					 String senderTitle){
	...
	private boolean sendMessage(String messageText){
		...
		try{
			...
			message.setStringProperty("sender", this.senderTitle);
			producer.send(message);
			// session.commit(); // may be does not need 
			connection.close();
