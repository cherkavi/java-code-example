...
public class SiteStatsDaoRedisImpl implements SiteStatsDao {
    ...
    private void updateOptimized(Jedis jedis, String key, MeterReading reading) {
        ...
        CompareAndUpdateScript luaScript = new CompareAndUpdateScript(jedis);
        transaction = jedis.multi();
        luaScript.updateIfGreater(transaction, key, SiteStats.maxWhField, reading.getWhGenerated());
        luaScript.updateIfLess(transaction, key, SiteStats.minWhField, reading.getWhGenerated());
        luaScript.updateIfGreater(transaction, key, SiteStats.maxCapacityField, getCurrentCapacity(reading));
        transaction.exec();
    }
