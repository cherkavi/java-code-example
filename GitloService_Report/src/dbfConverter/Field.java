...
public class Field {
	...
	/** передать в качестве всех полей в таблице массив из полей
	 * @param fields - все поля таблицы 
	 * */
	public static Field[] ControlSqlFields(Field[] fields){
		// переименовать поля, которые имеют SQL имена - добавив к ним префикс "_ref"
		for(int counter=0;counter<fields.length;counter++){
			if(SqlKeyWords.isSqlKeywords(fields[counter].getSqlName())){
				//System.out.println("keyWords:"+fields[counter].getSqlName());
				fields[counter].setSqlName(fields[counter].getSqlName()+"_ref");
			}
		}
		// переименовать поля, которые имеют одинаковые имена
		for(int counter=0;counter<fields.length;counter++){
			if(getRepeatCount(fields, fields[counter])>1){
				fields[counter].setSqlName(fields[counter].getSqlName()+"_"+Integer.toString(counter));
			}
		}
		return fields;
	}
	
	/** получить кол-во повторений указанной строки в массиве 
	 * @param fields массив из полей 
	 * @param field поле для поиска
	 * */
	private static int getRepeatCount(Field[] fields,Field field){
		int returnValue=0;
		for(int counter=0;counter<fields.length;counter++){
			if(fields[counter].getSqlName().equals(field.getSqlName())){
				returnValue++;
			}
		}
		return returnValue;
