...
public class Main extends JFrame{
	...
	private final static String generatorPreambula="GEN_";
	private final static String generatorPostambula="_ID";
	...
	private void initComponents() {
		...
		JButton buttonCreateGenerators=new JButton("Create Generator's for Tables and put into Clipboard");
		...
		buttonCreateGenerators.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				onButtonCreateGenerator();
			}
		});
		// placing 
		...
		panelMain.add(buttonCreateGenerators);
	...
	/** создать генераторы для таблиц и скопировать их в буфер обмена*/
	private void onButtonCreateGenerator(){
		...
		try{
			...
			while(rs.next()){
				returnValue.append("CREATE GENERATOR ");
				returnValue.append(generatorPreambula);
				rs.getString("TABLE_NAME");
				returnValue.append(generatorPostambula);
				returnValue.append(";\n");
			}
			getToolkit().getSystemClipboard().setContents(new StringSelection(returnValue.toString()),null);
			JOptionPane.showMessageDialog(this, "Generator's into ClipBoard");
		}catch(Exception ex){
			JOptionPane.showMessageDialog(this, "Data create error\n"+ex.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
		}finally{
			try{
				connection.close();
			}catch(Exception ex){};
