 ...
 * 
 *******************************************************************************/
package com.lts.ipc.test.namedpipe;

import com.lts.ipc.IPCException;
import com.lts.ipc.namedpipe.NamedPipe;
import com.lts.ipc.test.TestException;
import com.lts.ipc.test.semaphore.ClientServerTest;

public class NamedPipeTest extends ClientServerTest
{
	private NamedPipe myNamedPipe;
	protected void performTest2() throws TestException
	{
		try
		{
			NamedPipe namedPipe = new NamedPipe("/temp/namedPipeFile");
			namedPipe.create();
		}
	...
	protected void setup()
	{
		...
		NamedPipe pipe = new NamedPipe(getName());
		setNamedPipe(pipe);
		byte[] buffer = new byte[getBufferSize()];
		setBuffer(buffer);
	}
	
	protected void setNamedPipe(NamedPipe pipe)
	{
		myNamedPipe = pipe;
	}
	
	protected NamedPipe getNamedPipe()
	{
		return myNamedPipe;
	}
