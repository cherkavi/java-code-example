...
class temp_thread extends Thread{
  volatile String message;// ключевое слово volatile сообщает компилятору что, возможно будут происходить изменения в "message" с помощью других потоков
  int delay_ms;
  ...
  public void run(){
     try{
        // т.к. this.message объявлена как "volatile" то это значит что в процессе выполнения, в данном случае в цикле,
        // значение this.message может изменить свое значение, и чтобы компилятор не сделал из него константу в данном блоке
        for(int i=0;i<5;i++){
