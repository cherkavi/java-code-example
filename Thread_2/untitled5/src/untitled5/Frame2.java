...
class resource
{
  // метод, при обращению к которому из потоков будет происходить защелка,
  //то есть если данный метод уже использует один из потоков, то второй просто будет ожидать пока первый не закончит работу с ним
