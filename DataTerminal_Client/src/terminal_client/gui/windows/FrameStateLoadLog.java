...
import terminal.function.RequestLogBoncard;
...
public class FrameStateLoadLog extends JInternalFrameParent
							   implements GetTask{
	/** */
	private static final long serialVersionUID = 1L;
	/** таблица логов */
	private JTable field_table_log;
	/** уникальное имя задачи загрузки лога  */
	private String field_function_log=this.addUniqueString("RequestLogBoncard");
	
	public FrameStateLoadLog(JInternalFrameParent parent, JDesktopPane desktop, 
	...
	protected void initComponents() {
		// create component's
		JButton button_load_log=new JButton("Загрузить лог закачек с сервера");
	...
	private void on_load_log_click(){
		if(!this.getPercentReport().isInAction(this.field_function_log,this.getRandomTailLength())){
			this.field_function_log=this.replaceUniqueString(this.field_function_log);
			(new RequestLogBoncard(this,
								   this.getPercentReport(),
								   this.field_function_log,
								   "Загрузка лога \"загруженных с сервера файлов\" ")
								   ).start(false);
		}else{
	...
	/** загрузить выделенный файл с сервера */
	private void on_load_file_click(){
		// TODO загрузить выбранный файл с сервера 
	}
