	...
	public void run(){
		...
		while(flagRun){
			try{
				...
				is.close();
				byte[] recievedByte=baos.toByteArray();
				logger.debug("socket successful readed:"+recievedByte.length);
				notifyData(recievedByte);
			}catch(Exception ex){
	...
	private void notifyData(byte[] data){
		...
		synchronized(this.queue){
			...
			while(this.getQueueSize()>this.limitQueueByte){
				this.logger.warn("SocketPortListener out of limit");
				this.queue.remove(0);
			}
			logger.debug("notify - данные получены ");
			this.queue.notify();
		}
	}
