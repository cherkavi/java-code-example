...
public class DatabaseExecutorAware implements IExecutorAware{
	...
	public IExecutor[] getExecutors() {
		logger.debug("получить все Executor-ы");
		...
		try{
			...
			while(rs.next()){
				logger.info("получен очередной Executor - обработать");
				IExecutor executor=this.readExecutorFromResultSet(rs);
				if(executor!=null){
					returnValue.add(executor);
					// INFO место получения времени очередного запуска элемента 
					...
					logger.info("следующий старт Executor-а after: "+sdf.format(nextStart));
			...
			logger.debug("установить для каждой запущенной записи новое время"); 
	...
	private IExecutor readExecutorFromResultSet(ResultSet rs){
		...
		logger.debug("получить из ResultSet Executor");
		try{
	...
	private boolean setAsExecuted(Connection connection, int id, int period, Date timeWrite){
		logger.debug("установить Executor как запущенный "+id);
		...
		try{
			...
			*/
			...
			if(statement.getString(1).equals("0")){
				...
				logger.error("процедура выполнена с ошибками Exception: "+statement.getString(4));
				returnValue=false;
			}
            statement.close();
			connection.commit();
			...
			try{
				// нельзя закрывать connection, потому что он может быть востребован на родительском уровне 
				//connection.close();
			}catch(Exception ex){};
		}
		return returnValue;
	...
	private String getQuery(){
		...
		// INFO Query получить все Jobber-ы, которые нужно выполнить 
