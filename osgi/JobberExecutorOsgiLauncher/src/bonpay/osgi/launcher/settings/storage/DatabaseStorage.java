...
public class DatabaseStorage implements IStorage{
	...
	public boolean loadAllRecords() {
		logger.debug("загрузить в хранилище все данные, данные - job_settings_all");
		boolean returnValue=false;
		Connection connection=this.connectionAware.getConnection();
		try{
			String query="select * from bc_admin.vc_ext_job_settings_all";
			logger.debug(query);
			...
			logger.error("ошибка при загрузке данных  Exception: "+ex.getMessage(),ex);
		}finally{
			try{
				connection.close();
			}catch(Exception ex){};
	...
	public boolean saveAllRecords() {
		logger.debug("сохранить все records по данному хранилищу данных ");
		boolean returnValue=false;
		Connection connection=this.connectionAware.getConnection();
		try{
			...
			logger.error("ошибка сохранения данных (из хранилища) Exception:"+ex.getMessage(), ex);
		}finally{
			try{
				connection.close();
			}catch(Exception ex){};
	...
	private void saveOrUpdate(Connection updateConnection, String key, Object value) throws Exception {
		...
		if(type!=null){
			...
			String query="{?= call BC_ADMIN.PACK_UI_EXT_JOB.set_setting(?,?,?,?)}";
			logger.debug("записать параметр в базу данных: "+query);
			CallableStatement statement = updateConnection.prepareCall(query);
			...
			if(statement.getString(1).equals("0")){
				//returnValue=true;
				logger.debug("параметр успешно записан ");
			}else{
				logger.error("ошибка записи параметра в базу данных Exception: "+statement.getString(4));
				//returnValue=false;
			}
            statement.close();
		}else{
		...
		for(int counter=0;counter<this.listOfConverter.size();counter++){
			if(this.listOfConverter.get(counter).typeEquals(type)){
				try{
					...
					logger.error("ошибка преобразования объекта в строку Exception:"+ex.getMessage());
					return null;
				}
			}
		}
		logger.warn("не найден тип для преобразования");
		return null;
	}
	...
	private String getTypeFromObject(Object value){
		...
		logger.warn("не опознан тип объекта"); 
		return null;
	}
